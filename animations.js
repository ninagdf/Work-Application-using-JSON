...
function convertToJson(event) {
  ...
  let formData = {
      ...
      tel: document.getElementById('phone-input').value,
      ...
      veteranStatus: document.querySelector('input[name="veteranStatus"]:checked').value,
      ...
      addressTextarea: document.getElementById('addressTextarea').value,
  ...
  if (resumeInput.files.length > 0) {
      formData.resume = resumeInput.files[0].name; // Only get the file name for now
  } else {
  ...
  form.method = 'POST';
  form.action = 'process_form.php'; // Your PHP script URL

  // Create hidden input fields to hold the form data
...
document.getElementById('registrationForm').addEventListener('submit', convertToJson);
